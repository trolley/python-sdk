...
class TestRecipient(unittest.TestCase):
    ...
    def test_retrieve_recipient(self):
        ...
        response = trolley.recipient.Recipient.find("R-91XQ4QBJ65W1U")
    ...
    def test_retrieve_recipient_Invalid_RecipientId(self):
        with self.assertRaises(trolley.exceptions.notFoundException.NotFoundException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.recipient.Recipient.find("jfjfj")
    @patch('paymentrails.recipient.Recipient.find', fake_find)
    def test_retrieve_recipient_None_Recipient(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.recipient.Recipient.find(None)
    ...
    def test_create_recipient(self):
        ...
        response = trolley.recipient.Recipient.create(body)
        status = {"ok": "true"}
    ...
    def test_create_recipient_None_body(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.recipient.Recipient.create(None)
    ...
    def test_update_recipient(self):
        ...
        response = trolley.recipient.Recipient.update(recipientId, body)
    ...
    def test_update_recipient_None_recipient(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            ...
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            body = {'firstName': 'Mark'}
            response = trolley.recipient.Recipient.update(None, body)
    @patch('paymentrails.recipient.Recipient.update', fake_update)
    def test_update_recipient_None_body(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.recipient.Recipient.update("R-hghgh", None)
    @patch('paymentrails.recipient.Recipient.delete', fake_delete)
    def test_delete_recipient(self):
        trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
        trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
        recipientId = 'R-91XPYX3V2MM1G'
        response = trolley.recipient.Recipient.delete(recipientId)
    ...
    def test_list_allRecipientsWithQueries_None_term(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.recipient.Recipient.search(1, 10, None)

    @patch('paymentrails.recipient.Recipient.find', fake_find)
    def test_retrieveLogs(self):
        trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
        trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
        response = trolley.recipient.Recipient.find("R-91XPYX3V2MM1G", "logs")
    ...
    def test_retrieveLogs_InvalidRecipientId(self):
        with self.assertRaises(trolley.exceptions.notFoundException.NotFoundException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.recipient.Recipient.find("wdwd", "logs")
    @patch('paymentrails.recipient.Recipient.find', fake_find)
    def test_retrievePayments(self):
        trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
        trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
        response = trolley.recipient.Recipient.find("R-91XPYX3V2MM1G", "payments")
    ...
    def test_retrievePayments_InvalidRecipientId(self):
        with self.assertRaises(trolley.exceptions.notFoundException.NotFoundException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.recipient.Recipient.find("wdwd", "payments")
