...
import trolley.types.recipient
...
def fake_find(recipientId, term=""):
    if recipientId is None:
        raise trolley.exceptions.invalidFieldException.InvalidFieldException("Recipient id cannot be None")
    if recipientId[0:1] != "R":
        raise trolley.exceptions.notFoundException.NotFoundException("Recipient id is invalid")
    if term == "logs":
...
class TestRecipient(unittest.TestCase):
    ...
    @patch('trolley.types.recipient.Recipient.find', fake_find)
    def test_retrieve_recipient(self):
        trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
        trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
        response = trolley.types.recipient.Recipient.find("R-91XQ4QBJ65W1U")
    ...
    def test_retrieve_recipient_Invalid_RecipientId(self):
        with self.assertRaises(trolley.exceptions.notFoundException.NotFoundException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.types.recipient.Recipient.find("jfjfj")
    @patch('trolley.types.recipient.Recipient.find', fake_find)
    def test_retrieve_recipient_None_Recipient(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.types.recipient.Recipient.find(None)

    @patch('trolley.types.recipient.Recipient.create', fake_create)
    def test_create_recipient(self):
        trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
        trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
        body = {'type': 'individual', 'firstName': 'Bob',
                'lastName': 'TheBuilder', 'email': 'bob@thebuilder.com'}
        response = trolley.types.recipient.Recipient.create(body)
        status = {"ok": "true"}
    ...
    def test_create_recipient_None_body(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.types.recipient.Recipient.create(None)
    
    @patch('trolley.types.recipient.Recipient.update', fake_update)
    def test_update_recipient(self):
        trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
        trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
        body = {'firstName': 'Mark'}
        recipientId = 'R-91XQ0FRA3T40Y'
        response = trolley.types.recipient.Recipient.update(recipientId, body)
    ...
    def test_update_recipient_None_recipient(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            ...
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            body = {'firstName': 'Mark'}
            response = trolley.types.recipient.Recipient.update(None, body)
    @patch('trolley.types.recipient.Recipient.update', fake_update)
    def test_update_recipient_None_body(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.types.recipient.Recipient.update("R-hghgh", None)
    @patch('trolley.types.recipient.Recipient.delete', fake_delete)
    def test_delete_recipient(self):
        trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
        trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
        recipientId = 'R-91XPYX3V2MM1G'
        response = trolley.types.recipient.Recipient.delete(recipientId)
    ...
    def test_delete_recipient_None_recipient(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.types.recipient.Recipient.delete(None)

    @patch('trolley.types.recipient.Recipient.search', fake_search)
    def test_list_allRecipientsWithQueries(self):
        trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
        trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
        response = trolley.types.recipient.Recipient.search(1, 10, "")
    ...
    def test_list_allRecipientsWithQueries_None_term(self):
        with self.assertRaises(trolley.exceptions.invalidFieldException.InvalidFieldException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.types.recipient.Recipient.search(1, 10, None)

    @patch('trolley.types.recipient.Recipient.find', fake_find)
    def test_retrieveLogs(self):
        trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
        trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
        response = trolley.types.recipient.Recipient.find("R-91XPYX3V2MM1G", "logs")
    ...
    def test_retrieveLogs_InvalidRecipientId(self):
        with self.assertRaises(trolley.exceptions.notFoundException.NotFoundException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.types.recipient.Recipient.find("wdwd", "logs")
    @patch('trolley.types.recipient.Recipient.find', fake_find)
    def test_retrievePayments(self):
        trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
        trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
        response = trolley.types.recipient.Recipient.find("R-91XPYX3V2MM1G", "payments")
    ...
    def test_retrievePayments_InvalidRecipientId(self):
        with self.assertRaises(trolley.exceptions.notFoundException.NotFoundException):
            trolley.configuration.Configuration.set_public_key(TestRecipient.public_key)
            trolley.configuration.Configuration.set_private_key(TestRecipient.private_key)
            response = trolley.types.recipient.Recipient.find("wdwd", "payments")
